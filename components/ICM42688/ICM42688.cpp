...
int ICM42688::readRegisters(uint8_t subAddress, uint8_t count, uint8_t* dest) {
    uint8_t out_buf[1];
    out_buf[0] = subAddress;
    ESP_ERROR_CHECK(i2c_master_transmit_receive(_dev_handle, out_buf, sizeof(out_buf), dest, count, I2C_TICKS_TO_WAIT));
    return 1;
}
